...
{
    public partial class ADD0715E_JABO : Form
    {
        ...
        public ADD0715E_JABO(String user, String pwd, String prjcd, String addpara)
            : this()
        {
            m_User = user;
            m_Pwd = pwd;
        ...
        private void Query()
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(strConn))
            {
                ...
                string sql = "";
                sql = "SELECT * FROM TIE_N1101 WHERE DEMNO='" + txtDemno.Text.ToString() + "'";
                MetroLib.SqlHelper.GetDataReader(sql, conn, delegate(OleDbDataReader reader)
                {
                ...
                sql = "SELECT * FROM TIE_N1102 WHERE DEMNO='" + txtDemno.Text.ToString() + "' ORDER BY LNO";
                MetroLib.SqlHelper.GetDataReader(sql, conn, delegate(OleDbDataReader reader)
                {
        ...
        {
            ADD0715E_JABO_01 f = new ADD0715E_JABO_01();
            f.ShowDialog(this);
        ...
        private void printableComponentLink_CreateReportHeaderArea(object sender, DevExpress.XtraPrinting.CreateAreaEventArgs e)
        {
            ...
            e.Graph.DrawString("[자보]보완자료 요청내역서", Color.Black, new RectangleF(0, 10, 1080, 40), DevExpress.XtraPrinting.BorderSide.None);
            // 조회조건
        ...
        private void printableComponentLink_CreateReportFooterArea(object sender, DevExpress.XtraPrinting.CreateAreaEventArgs e)
        {
            ...
            e.Graph.DrawString("ADD0715E_JABO", Color.Black, new RectangleF(0, 0, 90, 20), DevExpress.XtraPrinting.BorderSide.None);
            // 출력일시(사실은 조회일시)
            e.Graph.Font = new Font("굴림", 8F, System.Drawing.FontStyle.Regular);
