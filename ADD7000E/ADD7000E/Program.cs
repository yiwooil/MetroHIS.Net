...
namespace ADD7000E
{
    static class Program
    {
        ...
        static void Main(String[] args)
        {
            ...
            if (args.Length > 0)
            {
                ParseArg(args[0], ref demno, ref user, ref pwd);
            }

            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            Application.Run(new ADD7000E(demno, user, pwd));
        }

        private static void ParseArg(String arg, ref String demno, ref String user, ref String pwd)
        {
            String[] aryArg = (arg + ',').Split(',');
            for (int i = 0; i < aryArg.Length; i++)
            {
                String[] val = (aryArg[i] + '=').Split('=');
                if ("DEMNO".Equals(val[0].ToUpper())) demno=val[1];
                else if ("USER".Equals(val[0].ToUpper())) user = val[1];
