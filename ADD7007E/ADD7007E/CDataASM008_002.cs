...
namespace ADD7007E
{
    public class CDataASM008_002 : CData, IData, IDataRemake
    {
        ...
        public void ReadDataFromEMR(OleDbConnection p_conn, OleDbTransaction p_tran)
        {
            ...
            // A. 기본정보
            RECU_EQP_ADM_YN = "2";           // 요양병원 여부(1.Yes 2.No)
            IPAT_OPAT_TP_CD = IOFG == "2" ? "1" : "2";           // 진료형태(1.입원 2.외래)
            IPAT_DT = A04_BEDEDT + A04_BEDEHM;                   // 입원일시(CCYYMMDDHHMM)
            if (IOFG == "2") DSCG_YN = A04_BEDODT != "" ? "1" : "2";                   // 퇴원여부(1.Yes 2.No)
            DSCG_FRM_CD = "";               // 퇴원형태 코드(01.정상퇴원 02.자의퇴원 03.탈원 04.가망없는 퇴원 05.전원 06.사망 99.기타)
            if (IOFG == "2")
            {
                if (A04_BEDODIV == "1") DSCG_FRM_CD = "01";
                else if (A04_BEDODIV == "2") DSCG_FRM_CD = "01";
                else if (A04_BEDODIV == "3") DSCG_FRM_CD = "01";
                else if (A04_BEDODIV == "4") DSCG_FRM_CD = "03";
            }
            VST_STAT_CD = "";               // 내원상태 코드(01.계속 외래 내원 02.귀 원에 입원 03.타 원에 입원 04.타 기관으로 전원 05.사망 99.기타)

            string sql = "";
            sql = "";
            sql += Environment.NewLine + "SELECT MIN(HEMDT) FSTDT";
            sql += Environment.NewLine + "  FROM TU12A (NOLOCK)";
            sql += Environment.NewLine + " WHERE HEMDT>='" + FR_DATE + "'";
            sql += Environment.NewLine + "   AND HEMDT<='" + TO_DATE + "'";
            ...
            MetroLib.SqlHelper.GetDataRow(sql, p_conn, p_tran, delegate(DataRow row)
            {
                ...
                BLDD_FST_STA_DD = row["FSTDT"].ToString(); // 최초 혈액투석 시작일
            ...
            MetroLib.SqlHelper.GetDataRow(sql, p_conn, p_tran, delegate(DataRow row)
            {
                ...
                BLDD_HOFC_FST_STA_DD = row["FSTDT"].ToString(); // 본원 최초 시작일
                return MetroLib.SqlHelper.BREAK;
            });

            // B. 환자 정보
            CHRON_RNFL_CUZ_SICK_CD = "";    // 만성신부전 원인 상병코드(1.당뇨병 2.고혈압 3.사구체신염 4.모름 9.기타)

            HRT_CCM_SICK_YN = "";           // 심장질환 여부(1.Yes 2.No)
            HRT_SICK_SYM = "";              // 상병분류기호
            CDFL_YN = "";                   // 심부전 여부(1.Yes 2.No)
            CDFL_CD = "";                   // 심부전 상세 코드(01.해당없음 02.좌심실 구혈율 03.심흉곽비)
            LVEF_CNT = "";                  // 좌심실 구혈율
            CT_RT_CNT = "";                 // 심흉곽비
            ATFB_YN = "";                   // 심방조동 여부(1.Yes 2.No)
            ATFB_CD = "";                   // 심방 세동 상세
            ISMA_HRT_DS_YN = "";            // 허혈성 심장병 여부(1.Yes 2.No)
            ISMA_HRT_DS_CD = "";            // 허혈성 심장병 상세
            OHS_YN = "";                    // 개심수술 여부
            ...
            MetroLib.SqlHelper.GetDataRow(sql, p_conn, p_tran, delegate(DataRow row)
            {
                ...
                HEIG = "";                      // 신장
                ASM_DLYS_BWGT = MetroLib.StrHelper.Round(row["LASTWT"].ToString(), 1).ToString();            // 건체중 값
                DLYS_BWGT_YN = ASM_DLYS_BWGT != "" ? "1" : "2"; // 건체중 측정 여부(1.Yew 2.No)
                ASM_BF_BWGT = MetroLib.StrHelper.Round(row["HMBECURWT"].ToString(), 1).ToString();           // 투석 전 체중
                BF_BWGT_YN = ASM_BF_BWGT != "" ? "1" : "2";     // 투석 전 체중 측정 여부(1.Yew 2.No)
                ASM_AF_BWGT = MetroLib.StrHelper.Round(row["HMAFCURWT"].ToString(), 1).ToString();           // 투석 후 체중
                AF_BWGT_YN = ASM_AF_BWGT != "" ? "1" : "2";     // 투석 후 체중 측정 여부(1.Yew 2.No)
            ...
            sql += Environment.NewLine + " WHERE PID='" + PID + "'";
            sql += Environment.NewLine + "   AND WDATE='" + STEDT + "'";
            sql += Environment.NewLine + "   AND ISNULL(CHGDT,'')=''";
            ...
            MetroLib.SqlHelper.GetDataRow(sql, p_conn, p_tran, delegate(DataRow row)
            {
                ...
                BLDD_PPRT_ASM_YN = "";          // 혈액투석 적절도 평가 여부
                BLDD_PPRT_ASM_CD = "";          // 적절도 평가 항목 코드
                PPRT_NUV = MetroLib.StrHelper.Round(row["SPKT"].ToString(), 3).ToString();                 // spKt/V 수치
                BLUR_DCR_RT = MetroLib.StrHelper.Round(row["URR"].ToString(), 1).ToString();               // URR 수치(%)
