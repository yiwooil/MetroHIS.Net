...
namespace ADD7007E
{
    class CQuery_ASM003 : CQuery
    {
        public List<CDataASM003_002> Query_ASM003(OleDbConnection conn, string frdt, string todt)
        {
            m_dic_cnectdd.Clear();
            m_dic_dcount.Clear();
            m_dic_billsno.Clear();
            ...
            List<CDataASM003_002> list = new List<CDataASM003_002>();
            ...
            MetroLib.SqlHelper.GetDataRow(sql, conn, delegate(DataRow row)
            {
                ...
                // 사용가능한 자료인지 점검
                if (Check_ASM003(row, conn) == true)
                {
                    CDataASM003_002 data = new CDataASM003_002();
                    data.Clear();
        ...
        private bool Check_ASM003(DataRow p_row, OleDbConnection p_conn)
        {
            System.Windows.Forms.Application.DoEvents();
            return true;
        }
