...
namespace ADD7007E
{
    class CQuery_ASM014 : CQuery
    {
        public List<CDataASM014_001> Query_ASM014(OleDbConnection conn, string frdt, string todt)
        {
            m_dic_cnectdd.Clear();
            m_dic_dcount.Clear();
            m_dic_billsno.Clear();
            ...
            List<CDataASM014_001> list = new List<CDataASM014_001>();
            ...
            MetroLib.SqlHelper.GetDataRow(sql, conn, delegate(DataRow row)
            {
                ...
                // 사용가능한 자료인지 점검
                if (Check_ASM014(row, conn) == true)
                {
                    CDataASM014_001 data = new CDataASM014_001();
                    data.Clear();
        ...
        private bool Check_ASM014(DataRow p_row, OleDbConnection p_conn)
        {
            System.Windows.Forms.Application.DoEvents();
            return true;
        }
