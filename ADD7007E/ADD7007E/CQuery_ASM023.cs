...
namespace ADD7007E
{
    class CQuery_ASM023 : CQuery
    {
        public List<CDataASM023_002> Query_ASM023(OleDbConnection conn, string frdt, string todt)
        {
            m_dic_cnectdd.Clear();
            m_dic_dcount.Clear();
            m_dic_billsno.Clear();
            ...
            List<CDataASM023_002> list = new List<CDataASM023_002>();
            ...
            MetroLib.SqlHelper.GetDataRow(sql, conn, delegate(DataRow row)
            {
                ...
                // 사용가능한 자료인지 점검
                if (Check_ASM023(row, conn) == true)
                {
                    CDataASM023_002 data = new CDataASM023_002();
                    data.Clear();
        ...
        private bool Check_ASM023(DataRow p_row, OleDbConnection p_conn)
        {
            System.Windows.Forms.Application.DoEvents();
            return true;
        }
