...
namespace ADD7006E
{
    class CHiraRNO001
    {
        ...
        public CHiraRNO001(string p_hosid, string p_demno, string p_cnecno, string p_cnectdd, string p_billsno)
        {
            m_Hosid = p_hosid;
            m_Demno = p_demno;
            m_Cnecno = p_cnecno;
        ...
        {
            if (data.RNO001_LIST.Count < 1) return;

            for (int idx = 0; idx < data.RNO001_LIST.Count; idx++)
            {
                ...
                // 문서 공통 정보
                dynReq.Metadata["SUPL_DATA_FOM_CD"].Value = "RNO001"; //서식코드: 중환자실기록자료
                ...
                // A.기본정보
                dynReq.Elements["DGSBJT_CD"].Value = data.RNO001_LIST[idx].INSDPTCD; // 진료과목
                dynReq.Elements["IFLD_DTL_SPC_SBJT_CDT"].Value = data.RNO001_LIST[idx].INSDPTCD2; // 내과 세부진료과목
                ...
                dynReq.Tables["TBL_CARE_RCD"].Columns.Add("NURSE_NM"); // 간호사성명
                for (int i = 0; i < data.RNO001_LIST.Count; i++)
                {
                    dynReq.Tables["TBL_CARE_RCD"].AddRow();
                    dynReq.Tables["TBL_CARE_RCD"].Rows[0]["RCD_DT"].Value = data.RNO001_LIST[i].WDTM; // 기록일시
                    dynReq.Tables["TBL_CARE_RCD"].Rows[0]["RCD_TXT"].Value = data.RNO001_LIST[i].RESULT; // 간호기록
                    dynReq.Tables["TBL_CARE_RCD"].Rows[0]["NURSE_NM"].Value = data.RNO001_LIST[i].PNURSE_NM; // 간호사성명
                }
                ...
                // 전송내역을 저장한다.
                SaveToTI86_RNO001(data, isTmp, p_sysdt, p_systm, p_conn);
            }

        }

        private void SaveToTI86_RNO001(CData data, bool isTmp, string p_sysdt, string p_systm, OleDbConnection p_conn)
        {
        }
    }
}
