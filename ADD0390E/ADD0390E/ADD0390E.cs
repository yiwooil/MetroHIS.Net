...
namespace ADD0390E
{
    public partial class ADD0390E : Form
    {
        ...
        private void SaveBoOrChu(string p_iofg, string p_k1, string p_k2, string p_k3, string p_k4, string p_k5, string p_k6, string p_simno, string p_addz1, string p_chugafg, OleDbConnection p_conn, OleDbTransaction p_tran)
        {
            ...
            // 추가청구이면 생성 원본인 A명세서의 키를 B명세서 XDAYS2에 저장한다.
            // 입원: BDODT,QFYCD,JRBY,PID,UNISQ,SIMCS
            // 외래: EXDATE,QFYCD,JRBY,PID,UNISQ,SIMCS
            string keyList = string.Join(",", new string[]
            {
                p_k1, p_k2, p_k3, p_k4, p_k5, p_k6
            });
            string new_xdays2 = "XDAYS2";
            if (p_addz1 == "2")
            {
                new_xdays2 = "'" + keyList.Replace("'", "''") + "'";
            }
